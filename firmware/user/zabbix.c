...
void sendKeyVal(char *key, char *val) {
	...
	base64_encode(strlen(key), (const unsigned char *)key, sizeof(base64key), base64key);
	base64_encode(strlen(val), (const unsigned char *)val, sizeof(base64value), base64value);
	...
	payload->data = (unsigned char *)os_zalloc(len*sizeof(char));
...
void sendToZabbix() {
	...
	base64_encode(sizeof(WIFI_AP_NAME)-1, (const unsigned char *)WIFI_AP_NAME, sizeof(base64host), base64host);
	char tmp[16];

	// uptime
	os_sprintf((char *)&tmp, "%lu", currentStatus. uptimeSeconds);
